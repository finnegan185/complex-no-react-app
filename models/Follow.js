...
Follow.prototype.validate = async function (action) {
  ...
  if (action == "delete") {
    if (!doesFollowAlreadyExist) {
      this.errors.push("You are not following this user so cannot unfollow them.");
    }
  }
...
Follow.getFollowsById = function (id) {
  return new Promise(async (resolve, reject) => {
    try {
      let follows = await followsCollection
        .aggregate([
          { $match: { authorId: id } },
...
Follow.countFollowersById = function (id) {
  return new Promise(async (resolve, reject) => {
    try {
      let followerCount = await followsCollection.countDocuments({ followedId: new ObjectId(id) });
...
Follow.countFollowingById = function (id) {
  return new Promise(async (resolve, reject) => {
    try {
      let followingCount = await followsCollection.countDocuments({ authorId: new ObjectId(id) });
