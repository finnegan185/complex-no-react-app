...
export default class Search {
  ...
  renderResultsHTML(posts) {
    if (posts.length) {
      ...
      <div class="list-group-item active" style="background-color:#6610f2; border-color: #6610f2"><strong>Search Results</strong> (${posts.length} ${posts.length == 1 ? "item" : "items"} found)</div>
      ${posts
        .map((post) => {
  ...
  createPostHTML(post) {
    ...
    return `<a href="/post/${post._id}" class="list-group-item list-group-item-action">
          ...
          <span class="text-muted small">by ${post.author.username} on ${dateFormatted}</span>
        </a>`;
  }
